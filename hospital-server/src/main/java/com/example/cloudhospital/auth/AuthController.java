...
@RestController @RequestMapping("/api/v1/auth") public class AuthController {
 ...
 @PostMapping("/doctor-register") public ApiResponse<AuthService.Session> doctorRegister(@Valid @RequestBody DoctorRegister r,HttpServletResponse response){AuthService.Session session=auth.registerDoctor(new AuthService.DoctorRegisterRequest(r.username(),r.password(),r.realName(),r.departmentName(),r.title()));TokenCookie.write(response,session.token());return ApiResponse.ok(session);}
 public record Login(@NotBlank String username,@NotBlank String password){} public record Register(@NotBlank String username,@Size(min=6) String password,@NotBlank String idCard,@NotBlank String name,@Pattern(regexp="MALE|FEMALE") String gender,LocalDate birthday,@NotBlank String phone,String address){} public record DoctorRegister(@NotBlank String username,@Size(min=6) String password,@NotBlank String realName,@NotBlank String departmentName,@NotBlank String title){}
}
