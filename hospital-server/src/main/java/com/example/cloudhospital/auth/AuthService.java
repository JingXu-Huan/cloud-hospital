...
public class AuthService {
  private final AuthMapper authMapper; private final PatientRepository patients; private final DoctorRepository doctors;
  public AuthService(AuthMapper authMapper, PatientRepository patients, DoctorRepository doctors){this.authMapper=authMapper;this.patients=patients;this.doctors=doctors;}
  ...
  @Transactional public Session registerDoctor(DoctorRegisterRequest r){
    if(authMapper.existsByUsername(r.username()) || doctors.findByLoginName(r.username()).isPresent()) throw new BizException(40003,"用户名已存在");
    Doctor doctor=new Doctor(); doctor.doctorNo="D"+LocalDate.now().toString().replace("-","")+String.format("%06d",new Random().nextInt(1_000_000)); doctor.loginName=r.username(); doctor.passwordHash="{managed-by-user-account}"; doctor.realName=r.realName(); doctor.departmentName=r.departmentName(); doctor.title=r.title(); doctor.enabled=true;
    doctors.save(doctor);
    String token=UUID.randomUUID().toString(); authMapper.insertDoctorAccount(r.username(),r.password(),token); Long id=authMapper.findIdByUsername(r.username());
    return new Session(id,r.username(),"DOCTOR",null,doctor.id,doctor.realName,doctor.departmentName,doctor.title,token);
  }
  public boolean usernameExists(String username){return authMapper.existsByUsername(username);}
  public void provisionDoctorAccount(String username){authMapper.insertDoctorAccount(username,"123456",null);}
  ...
  public record DoctorRegisterRequest(String username,String password,String realName,String departmentName,String title){}
}
