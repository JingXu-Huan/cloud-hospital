...
const load = async () => {
  rows.value = await http.get("/prescriptions");
};
const manualPay = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确认已在院内收取 ${row.patientName} 的 ¥${Number(row.totalAmount).toFixed(2)}？此操作仅适用于尚未注册患者端的患者。`,
      "人工确认收款",
      {
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(
    (row) =>
      ...
      (!query ||
        [row.prescriptionNo, row.patientName, row.doctorName].some((value) =>
          String(value || "")
            .toLowerCase()
            .includes(query),
        )),
...
watch(filteredRows, (records) => {
  currentPage.value = Math.min(
    currentPage.value,
    Math.max(1, Math.ceil(records.length / pageSize.value)),
  );
});
...
<template>
  <div class="page">
    <div class="toolbar">
      ...
      <span class="hint"
        >已注册患者在本人端支付；未注册患者可由院内确认收款。</span
      >
    </div>
      ...
      <el-table class="desktop-table" :data="pagedRows">
        ...
        <el-table-column label="操作" min-width="150"
          ><template #default="scope"
            ><el-button
              v-if="
                scope.row.status === 'UNPAID' && !scope.row.patientRegistered
              "
              type="primary"
              size="small"
              @click="manualPay(scope.row)"
              >确认院内收款</el-button
            ><span
              v-else-if="scope.row.status === 'UNPAID'"
              class="patient-pay-tip"
              >患者端支付</span
      ...
      <div class="mobile-record-list">
        <article v-for="row in pagedRows" :key="row.id" class="mobile-record">
          ...
          <div class="mobile-record-meta">
            ...
            ><span>{{ row.doctorName }} · {{ row.prescribedAt }}</span>
          </div>
          <div class="mobile-record-actions">
            <span class="amount">¥{{ Number(row.totalAmount).toFixed(2) }}</span
            ><el-button
              v-if="row.status === 'UNPAID' && !row.patientRegistered"
              type="primary"
              size="small"
              @click="manualPay(row)"
              >确认院内收款</el-button
            ><span v-else-if="row.status === 'UNPAID'" class="patient-pay-tip"
              >请在患者端完成支付</span
            >
          </div>
        </article>
