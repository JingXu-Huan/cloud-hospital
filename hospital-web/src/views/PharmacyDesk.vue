...
const dispense=async row=>{try{await ElMessageBox.confirm(`确认向 ${row.patientName} 发药？`,'发药核对',{type:'warning'});await http.post(`/prescriptions/${row.id}/dispense`);ElMessage.success('发药成功');load()}catch(e){if(e==='cancel'||e==='close')return}}
onMounted(load)
</script>
